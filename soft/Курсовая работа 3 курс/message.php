...
<?php
	...
	$result = mysql_query("SELECT `id`, `message`, `from`, `to`, DATE_FORMAT(`send`, '%H:%i %d.%m.%Y') as send FROM `message` WHERE `from` = '{$userid}' OR `to` = '{$userid}' ORDER BY `send` DESC", $db) or die("Не могу выполнить запрос.");
...
<?php while($msg = mysql_fetch_assoc($result)) { ?>
	<tr>
		...
		<td><?php echo htmlspecialchars($msg['send']); ?></td>
...
while($man = mysql_fetch_assoc($result)) {
	echo '<option value="' . htmlspecialchars($man['id']) . '">' . htmlspecialchars($man['firstname']) . ' ' . htmlspecialchars($man['lastname']) . '</option>';
}
?>
			</select><br>
