...
<?php
		...
		$result = mysql_query("SELECT `id`, `message`, DATE_FORMAT(`send`, '%H:%i %d.%m.%Y') as sendtime, `author` FROM `post` WHERE `theme` = '{$themeid}' ORDER BY `send` DESC", $db) or die("Не могу выполнить запрос.");
...
<tr>
	...
	<td><?php echo htmlspecialchars($post['sendtime']); ?></td>
